...
\begin{figure}[th!]
  ...
  badge/.style={font=\scriptsize\itshape, anchor=north east, fill=white, draw=gray!45, rounded corners=2pt, inner sep=2.5pt},
  flow/.style={-{Stealth[length=2.6mm]}, gray!65, thick},
]
\def\w{16.5}
\fill[orange!8, draw=orange!55, rounded corners=5pt] (0,1.5) rectangle (\w,-1.5);
\fill[blue!8,   draw=blue!50,   rounded corners=5pt] (0,-1.9) rectangle (\w,-5.7);
\fill[teal!9,   draw=teal!55,   rounded corners=5pt] (0,-6.1) rectangle (\w,-9.0);
\fill[gray!12,  draw=gray!60,   rounded corners=5pt] (0,-9.4) rectangle (\w,-13.2);
\draw[flow] (1.9,-1.5) -- (1.9,-1.9);
\draw[flow] (1.9,-5.7) -- (1.9,-6.1);
\draw[flow] (1.9,-9.0) -- (1.9,-9.4);
...
\node[badge] at (\w-0.15,1.42) {Usability};
...
\node[badge] at (\w-0.15,-1.98) {Scalability + Fault Tolerance};
...
\node[q, anchor=west, text width=5.2cm] at (0.35,-8.2) {How do control and occupants\\observe and program actions?};
\node[badge] at (\w-0.15,-6.18) {Observability + Programmability};
...
\end{tikzpicture}}
  \caption{\bf \small The three thesis contributions as layers of the smart-space control stack.
    {\it Control in Context (\emph{usability}) characterizes how occupants view and use control interfaces;
    CoMesh (\emph{scalability \& fault tolerance}) decentralizes where control logic runs, across overlapping coteries of smart devices and with no central hub;
    and RASC (\emph{observability \& programmability}) exposes how control logic and occupants observe and program each action's lifecycle (Request--Ack--Start--Complete).
    All three operate over the same smart space of IoT devices, mesh network, and people.}}
  \bigskip
