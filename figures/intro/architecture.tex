...
\node[badge] at (\w-0.15,1.42) {\textbf{Management plane}\\\itshape usability};
...
\node[badge] at (\w-0.15,-1.98) {\textbf{Control plane}\\\itshape scalability \& fault tolerance};
...
\node[badge] at (\w-0.15,-6.18) {\textbf{Action plane}\\\itshape observability \& programmability};
\begin{scope}[yshift=-0.5cm]
...
\end{tikzpicture}}
  \caption{\bf \small Three user-aware planes of smart-space control, designed around the needs of the people who inhabit the space.
    {\it The \emph{management plane} (Control in Context) characterizes how occupants view and use control interfaces; the \emph{control plane} (CoMesh) decentralizes where control logic runs; and the \emph{action plane} (RASC) exposes each action's Request--Ack--Start--Complete lifecycle. All three operate over a shared smart space of devices, mesh network, and people.}}
  \bigskip
