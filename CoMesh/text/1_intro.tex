...
Commercial edge deployments with Internet of Things (IoT) devices are rapidly growing in size and density. There are around 21 B connected IoT devices today, forecast to reach roughly 40 B by 2030~\cite{iotanalytics2025}. Commercial settings are the next frontier beyond smart homes (\Cref{comesh:fig:IoTsetting}). For example, smart buildings already account for around 2 B devices, and are forecast to reach 4.12 B devices by 2030, with the building IoT market exceeding \$100 B~\cite{memoori2025iot}.
...
This STA abstraction is commonly programmed via {\it routines}, available in Samsung SmartThings \cite{SmartThings}, Amazon Alexa \cite{Alexa}, Google Home \cite{GoogleHome}, etc. A  routine is a sequence of {\it commands} that actuates multiple devices,  wherein each command executes an action on one device. Multiple routines may be active at any given time, while others remain dormant (waiting to be triggered). A routine may be triggered either based on sensor values (Boolean predicate), or at particular times, or a combination of these, or manually by a human.

\sysname{} reuses these familiar routine front-ends rather than introducing a new programming interface. \Cref{comesh:fig:prog-workflow} shows \sysname's programming workflow: a user authors a routine through an existing graphical user interface (GUI) such as the Samsung SmartThings app, which is exported as SmartThings' internal JSON; \sysname{} converts this to its own JSON specification, which is then promulgated to the smart devices that monitor and execute the routine. \sysname{} likewise integrates with Amazon Alexa and Google Home. Thus the abstraction a user sees is unchanged; \sysname{} only changes {\it how} the routine is managed underneath---in a decentralized, hub-less fashion.

\begin{figure}[tb!]
    \centering
    \includegraphics[width=0.95\linewidth,alt={Programming workflow diagram. On the left, a routine named ``Auto Irrigation'' is authored in the Samsung SmartThings app interface, with an ``If'' section listing soil-sensor humidity conditions and a ``Then'' section listing irrigation-actuator commands. A user icon in the middle connects the SmartThings interface to a box labeled \sysname{} on the right. Below, an arrow shows the SmartThings routine exported as SmartThings internal JSON, which is then converted into \sysname{}'s own JSON specification; both JSON snippets are shown side by side.}]{CoMesh/figures/intro/smartThings.png}
    \caption{\bf \small \sysname{} Programming Workflow. {\it Routines are authored in an existing GUI such as the Samsung SmartThings app, and SmartThings' internal JSON is converted to \sysname's JSON specification before being promulgated to devices.}}
    \medskip
    \label{comesh:fig:prog-workflow}
\end{figure}

This chapter focuses on management of such routine-based workloads running over large commercial edge IoT meshes. \Cref{comesh:fig:IoTsetting} (from \Cref{intro:sec:background}) shows three emerging edge mesh scenarios---a smart building (b), a smart farm (c), and a smart factory (d).
...
\begin{figure}[t]
    ...
    \begin{subfigure}[b]{0.4\linewidth}
        \centering
        \includegraphics[width=\textwidth,alt={Grouped bar chart with error bars. x-axis: number of devices (1, 4, 16, 64, 144); y-axis: CPU load percent (0 to 60). Three bars per group: \sysname{}, 3-Hubs, and Central. At 1 device all three are comparable (about 8 to 12 percent). As devices increase, \sysname{} stays flat and low (roughly 6 to 10 percent), while 3-Hubs rises to about 33 percent and Central climbs steeply to about 35 percent at 16 devices and 51 to 54 percent at 64 to 144 devices.}]{CoMesh/figures/intro/CPU_cmp_xdev_bar.pdf}
        \caption{ \small Fixed: 20 total routines.
        }
    ...
    \begin{subfigure}[b]{0.4\linewidth}
        \centering
        \includegraphics[width=\textwidth,alt={Grouped bar chart with error bars. x-axis: number of routines (1, 5, 10, 15, 20); y-axis: CPU load percent (0 to 60). \sysname{} stays flat at about 9 percent across all routine counts, 3-Hubs ranges from about 14 to 25 percent (rising with more routines), and Central stays highest at roughly 33 to 37 percent throughout.}]{CoMesh/figures/intro/CPU_cmp_xrtn_bar.pdf}
        \caption{\small Fixed: 16 total devices.}
        \label{comesh:subfig:cpu-cmp}
    \end{subfigure}
    \caption{\bf \small Raspberry Pis CPU utilization. {\it Our decentralized \sysname{} reduces CPU utilization significantly compared to the centralized Hub and 3-Hubs.}}
    \medskip
...
This chapter takes the next big step in making cloudless commercial edges hubless and fully decentralized. The cloudless+hubless approach addresses concerns of the hub being overwhelmed
...
\begin{table}[t]
\centering
\caption{\bf \small N-hubs vs.\ \sysname{} design trade-offs.}
    \bigskip
...
\hline
\textbf{Property} & \textbf{N-Hubs (Sharded/Hierarchical)} & \textbf{CoMesh (this chapter)} \\
\hline\hline
...
(i) operate {\it over the local edge mesh}, (ii)  remain {\it local}, without requiring a connection to the cloud, and (iii) maintain {\it fault tolerance\footnote{While this thesis only deals with crash failures, our decentralized approaches spread the attack surface and are more secure than a centralized cloudless solution. Security is beyond the scope of this thesis.}} in spite of local device failures.  
...
over large edge meshes. The contributions of this chapter are: 
\begin{itemize}
...
\begin{figure}[t!]
    \centering
    \includegraphics[width=0.8\linewidth,alt={Diagram of CoMesh's Sense-Trigger-Actuate control workflow over an ad-hoc mesh network. Sensing, triggering, and actuation responsibilities are handled by coteries of nearby devices rather than a central hub; ad-hoc network links are shown faintly in the background and all coteries are drawn as hyper-local.}]{CoMesh/figures/intro/Coteries.pdf}
    \caption{\bf \small Control Flow of \sysname's Sense-Trigger-Actuate Actions. {\it Ad-hoc network links are shown in the background. (For simplicity, all \kgroups{} are shown as hyper-local.)}
    }
    \medskip
