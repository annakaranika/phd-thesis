...
\begin{table}[tb!]
    \centering
    \footnotesize
    \caption{\bf \small Default parameter values for simulation.}
    \bigskip
...
\begin{figure}[ht]
    ...
    \caption{\bf \small Client Delay: Sequential (SLA) vs. Optimistic (OLA) Lock-Acquiring Strategy, in three topologies.
    {\it 1 time unit is equivalent to 1 ms of real time.}
    }
    \medskip
...
\begin{figure}[ht] % Use figure for spanning two columns in a two-column document
    ...
    \begin{minipage}{0.49\textwidth}
        ...
        \caption{\bf \small Client and Sync Delays of routines under failure vs\. Churn fraction and packet Drop rate (80\% smart devices).}
        \medskip
...
\begin{figure}[tb!]
    ...
    \caption{\bf \small Mobility: Client delay vs. relative distance moved.
    }
    \medskip
...
\begin{figure}[tb!]
    ...
    \begin{subfigure}[b]{0.4\linewidth}
        \centering
        \includegraphics[width=\textwidth,alt={CDF plot of temporal load under the LSH/locality policy. x-axis: temporal load in time units (0 to about 520,000); y-axis: cumulative probability (0 to 1). Three S-shaped curves with longer tails than under random: leader_only (blue, about 0 to 130,000), member_only (orange, about 100,000 to 380,000), and leader/member (green, about 150,000 to 520,000). Medians are close to those under random, but the tails are longer.}]{CoMesh/figures/exp_eval/temporal_load_balance_locality.png}
        \caption{\small \textit{Temporal} Load, LSH.}
        % \medskip
        \label{comesh:subfig:temporal-load-locality}
    \end{subfigure}
    \hspace{1cm}
    \begin{subfigure}[b]{0.4\linewidth}
        \centering
        \includegraphics[width=\textwidth,alt={CDF plot of spatial load under the LSH/locality policy. x-axis: number of concurrent \kgroups{} a device belongs to (0 to about 40); y-axis: cumulative probability (0 to 1). Three S-shaped curves by role: leader_only (blue, about 0 to 10), member_only (orange, about 0 to 25), and leader/member (green, about 0 to 35). The distributions are slightly more spread out than under random.}]{CoMesh/figures/exp_eval/spatial_load_balance_locality.png}
        \caption{\small \textit{Spatial} Load, LSH.}
        % \medskip
        \label{comesh:subfig:spatial-load-locality}
    \end{subfigure}
    \par\medskip
    \begin{subfigure}[b]{0.4\linewidth}
        \centering
        \includegraphics[width=\textwidth,alt={CDF plot of temporal load under the random policy. x-axis: temporal load in time units (0 to about 520,000); y-axis: cumulative probability (0 to 1). Three nearly vertical (low-variance) curves by role: leader_only (blue, around 40,000 to 55,000), member_only (orange, around 190,000 to 210,000), and leader/member (green, around 240,000 to 270,000). The steep curves indicate load is tightly balanced across devices.}]{CoMesh/figures/exp_eval/temporal_load_balance_random.png}
        \caption{\small \textit{Temporal} Load, Random.}
        % \medskip
        \label{comesh:subfig:temporal-load-random}
    \end{subfigure}
    \hspace{1cm}
    \begin{subfigure}[b]{0.4\linewidth}
        \centering
        \includegraphics[width=\textwidth,alt={CDF plot of spatial load under the random policy. x-axis: number of concurrent \kgroups{} a device belongs to (0 to about 40); y-axis: cumulative probability (0 to 1). Three S-shaped curves by role: leader_only (blue, about 0 to 7), member_only (orange, about 3 to 18), and leader/member (green, about 5 to 22).}]{CoMesh/figures/exp_eval/spatial_load_balance_random.png}
        \caption{\small \textit{Spatial} Load, Random.}
        % \medskip
        \label{comesh:subfig:spatial-load-random}
    \end{subfigure}
    \caption{\bf \small \sysname{}'s Load Distribution in Time \& Space: LSH/locality vs. Random (member \& leader selection). {\it Grid topology.}
    }
    \medskip
...
The plots show LSH's clustering of \kgroups{} closer to target devices prolongs tail load vs. random (\Cref{comesh:subfig:temporal-load-locality,comesh:subfig:spatial-load-locality}) due to repeated elections. Yet LSH's median temporal load for member and leader/member are similar to random's (\Cref{comesh:subfig:temporal-load-random}), showing LSH balances load well.
...
\begin{figure}[tb!]
    ...
    \begin{minipage}{0.49\textwidth}
        ...
        \caption{\bf \small Delays of operations within a \Kgroup{} for different $\K$.\\}
        % \medskip
    ...
    \begin{minipage}{0.49\textwidth}
        ...
        \caption{\bf \small Delays of operations within a \Kgroup{} for different policies of entity clustering, member selection, and leader election.}
        % \medskip
    ...
    \begin{minipage}{\textwidth}
        ...
        \caption{\bf \small \Kgroup{} Internal Operations vs. Smart Device  Churn.\\}
...
{The \Bldg{} (\Cref{comesh:fig:smart-building}) is a ``living laboratory'' sized 250 ft $\times$ 362 ft, with
...
\begin{figure}[tb!]
    ...
    \begin{subfigure}[b]{0.4\linewidth}
        ...
        \caption{\small CDF of how many devices a routine actuates. 
        }
        \medskip
    ...
    \begin{subfigure}[b]{0.57\linewidth}
        ...
        \caption{\small CDF of the number of conflicting actuated devices between routine pairs.
        }
        \medskip
    ...
    \begin{subfigure}[b]{\linewidth}
        ...
        \caption{\small Routine conflicts for the \SmallT{} (bottom right 3 classes) and \LargeT{} topologies. Circle = routine class; Color = trigger condition; Count = number of routines; Thicker edges = more conflicting devices.   
        }
        \medskip
        \label{comesh:fig:rtn_conflicts}
    \end{subfigure}
    \caption{\bf \small Workload in
    \Bldg{} Map-Driven Simulations. {\it \LargeT{} topology: all 545 routines.
...
For concreteness, we describe a few sample routine classes. When motion is undetected for at least an hour and a room's light is on, a routine turns it off; conversely, when motion is detected, another routine turns the room's lights on. Other routines turn off room and corridor lights overnight. These per-room routines conflict with building-wide routines---e.g., ones that turn off all lights everywhere between 2--6 AM---and similar local--global conflicting pairs arise for thermostats and shades. All routines are triggered by sensors.
...
\begin{figure}[th!]
    ...
    \caption{\bf \small \Bldg{} (Map-driven) Simulation (\Cref{comesh:fig:smart-building}): Wait times and Sync delays vs. device transmission range. {\it (1 time unit in simulation $\simeq$ 1 ms of real time). Upper end shows max. }
    }
    \medskip
