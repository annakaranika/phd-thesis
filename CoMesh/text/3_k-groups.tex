...
Bully election algorithm to quickly confirm the leader~\cite{Bully}. Bully election is attractive as it is fast in the common case: 1 round-trip time (RTT) for lowest-hashed \kgroup{} device (leader) to inform others in the \kgroup{}.
...
\begin{algorithm}[t]
    ...
    \begin{algorithmic}[1]
        ...
        \If{$S==L_{old}$}
            ...
            \State Receive and ack \texttt{CHANGEOVER} from $L_{\mathcal{D}}$ \Comment{(e)} \label{comesh:alg:migration:e}
            \State Send \texttt{CHANGEOVER} to $\mathcal{M}_{old}$  \Comment{(f)} \label{comesh:alg:migration:f}
...
that it has received all the state (line~\ref{comesh:alg:migration:e}), the old \kgroup{} leader can delete its old \kgroup{} state. After line~\ref{comesh:alg:migration:f}, an old \kgroup{} node can delete its old state for that \kgroup{}. During \Cref{comesh:alg:migration}'s execution,
...
Note that \Cref{comesh:alg:lsh_majority_selection} selects only the $(f{+}1)$-member {\it majority} of a \kgroup{} locally (via LSH); the remaining $f$ members are spread across the network by the base consistent hash (\Cref{comesh:subsection:member_selection}). This split is deliberate: keeping the $(f{+}1)$-quorum local makes the common-case intra-\kgroup{} operations fast, while spreading the other $f$ members ensures that a {\it correlated} failure of a single locality cannot wipe out an entire \kgroup. For example, an entire power domain may lose power at once; by our rule of thumb for setting $f$ (\Cref{comesh:sec:system-model}), a power domain holds fewer than $f{+}1$ smart devices, so at least one nearby \kgroup{} member always survives such an outage.
...
\begin{table}[t]
    \centering
    \caption{\bf \small Summary of LSH parameters used in \sysname.}
    \bigskip
...
\begin{figure}[tb!]
    ...
    \begin{subfigure}[b]{0.4\linewidth}
        \centering
        \includegraphics[width=\textwidth,alt={Three stacked line subplots of average quorum distance versus a varied LSH parameter (x-axis 1 to 6), each comparing the RANDOM policy (blue, flat at about 4.2) against  (orange). Top, varying m:  rises from about 1.5 to 3.5. Middle, varying l:  falls from about 2.2 to 1.5. Bottom, varying r:  falls from about 3.7 to 1.7.  keeps quorum distance below RANDOM in all cases.}]{CoMesh/figures/k-groups/500_grid5,10,10lsh_kdis_k_l_r.png}
        \caption{\small Average quorum distance.}
    ...
    \begin{subfigure}[b]{0.4\linewidth}
        \centering
        \hspace{-8pt}
        \includegraphics[width=1.04\textwidth,alt={Three stacked line subplots of average \kgroup{} overlap versus a varied LSH parameter (x-axis 1 to 6), each comparing the RANDOM policy (blue, flat near 0.1) against  (orange). Top, varying m:  falls from about 0.8 to 0.55. Middle, varying l:  rises from about 0.6 to 0.8. Bottom, varying r:  rises from about 0.4 to 0.8.  consistently yields much higher overlap (about 0.4 to 0.8) than RANDOM (about 0.1).}]{CoMesh/figures/k-groups/500_grid5,10,10lsh_koverlap_k_l_r.png}
        \caption{\small Average \kgroup{} overlap.}
        \label{comesh:subfig:overlap_500klr}
    \end{subfigure}
    \caption{\bf \small Average quorum distance and \kgroup{} overlap for different selection policies and parameters. {\it $N$ = 500, \#smart devices = 200.}}
    % \medskip
...
\begin{figure}[tb!]
    ...
    \begin{subfigure}[b]{0.42\linewidth}
        \centering
        \includegraphics[width=\textwidth,alt={Line plot of average quorum distance versus number of devices (50 to 1000). Two series: the RANDOM policy rises roughly linearly from about 2.4 at 50 devices to about 5.4 at 1000 devices, while the LSHMix policy stays low and nearly flat between about 1.7 and 2.0 across all system sizes.}]{CoMesh/figures/k-groups/50_grid5,5,2lsh_topology_distance.png}
        \caption{\small Average quorum distance.}
        \label{comesh:subfig:quorum_dis_size}
        \vspace{0.05cm}
    \end{subfigure}
    \hspace{0.5cm}
    \begin{subfigure}[b]{0.42\linewidth}
        \centering
        \includegraphics[width=\textwidth,alt={Line plot of average k-group overlap (the number of members shared between two consecutive epochs) versus number of devices (50 to 1000). Both policies decrease as the number of devices grows, but LSHMix stays consistently above random: LSHMix falls from about 1.8 to 0.6 while random falls from about 1.3 to below 0.1, so LSHMix retains more members across epochs at every system size.}]{CoMesh/figures/k-groups/50_grid5,5,2lsh_topology_overlaps.png}
        \caption{\small Average \kgroup{} overlap.}
        \label{comesh:subfig:overlap_size}
    \end{subfigure}
    \caption{\bf \small Average quorum distance and \kgroup{} overlap for different system sizes $N$. {\it 2D grid, default $m=2, l=2, r=4$, 40\% smart devices}.}
    \medskip
    \label{comesh:fig:lsh_vs_size}
\end{figure}
...
This has clear benefits across {\it system sizes}. \Cref{comesh:fig:lsh_vs_size} evaluates 2D grid topologies ranging from 50 to 1000 devices, with the default parameters. The average quorum distance (\Cref{comesh:subfig:quorum_dis_size}) grows roughly linearly with system size under random selection (from about 2.4 to 5.4), but stays small and nearly flat under LSHMix (about 1.7 to 2.0). The average \kgroup{} {\it overlap}---the number of members a \kgroup{} retains across two consecutive epochs, which lowers state-transfer cost at epoch changes---is consistently higher under LSHMix than random (\Cref{comesh:subfig:overlap_size}).

\begin{table}[tb!]
    \centering
    \caption{\bf \small Average number of \kgroup{} member candidates for different system sizes $N$ (40\% smart devices).}
    \bigskip
    \label{comesh:tab:lsh_candidates}
    \tagpdfsetup{table/header-rows={1},table/header-columns={1}}
    \footnotesize
    \begin{tabular}{l c c c c c}
        \hline
        \textbf{\# devices ($N$)} & 50 & 250 & 500 & 750 & 1000 \\ \hline
        \textbf{\# candidates} & 13.0 & 34.1 & 50.8 & 52.8 & 66.9 \\
        \hline
    \end{tabular}
    \bigskip
\end{table}

Although LSHMix's overlap drops for large $N$ (\Cref{comesh:subfig:overlap_size}), it remains well above random's. The drop is explained by \Cref{comesh:tab:lsh_candidates}: as the system grows, each \kgroup{} has more candidate members, so the probability of re-selecting the same device in consecutive epochs shrinks. One can also tune LSH parameters to reduce the candidate count without hurting quorum distance---e.g., at $N=1000$ with $m=3, l=2, r=4$, the average number of candidates is 21 and the average quorum distance is 2.4 (vs.\ 5.3 for random \kgroup{} selection).
