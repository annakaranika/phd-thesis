...
\subsubsection{Action Items for Smart-Home OS Developers}
\label{conc:sec:os-action-items}

Control in Context's findings translate directly into design guidance for builders of home management systems (``smart-home OSes''). Such systems should: (i)~\emph{disclose capability ceilings} and provide explicit deep-link \emph{bridges} to device-local controls when the hub cannot reach a feature; (ii)~make \emph{provenance and interoperability legible}, surfacing who or what executed an action and running pre-flight ``weakest-link'' checks before multi-brand routines; (iii)~provide a \emph{brand-agnostic integration layer} that adapts to whatever each vendor exposes rather than assuming full cooperation; (iv)~treat routines and devices as \emph{shared resources} with role-based access control and change histories for multi-user homes; and (v)~\emph{support transitions} along the control spectrum rather than locking users into a single scheme. A complementary open question is a principled decision framework for \emph{when} a deployment should decentralize its control plane, given device count, churn, latency budget, and privacy constraints.
...
Capable small language models now run on commodity hardware, enabling on-device natural-language interaction without the cloud. Building on CoMesh's edge-native infrastructure and RASC's observability, future work could distribute language-model inference across the heterogeneous, often-idle devices already present in a smart space~\cite{hyperedge}---adaptively choosing pipeline, tensor, or mixed-mode placement to fit tight memory and wireless budgets---for low-latency natural-language control that survives cloud or ISP outages.
