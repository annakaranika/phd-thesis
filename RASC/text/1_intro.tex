...
\begin{figure}[th]
    \centering
    \includegraphics[width=0.7\linewidth,alt={CDF plot with a compressed x-axis that uses axis breaks to show widely separated ranges of smart device action lengths. y-axis: CDF (0 to 1). Four curves: door (blue, 4 to 5 seconds, a tight steep S-curve), elevator (orange, 17 to 21 seconds), shade (green, 30 to 33 seconds), and thermostat (red, 125 to 3493 seconds, a long spread). Each action has a distinct, non-overlapping length range, and lengths span nearly three orders of magnitude across device types.}]{RASC/figures/intro/action-length-var.pdf}
    \caption{\bf \small IoT actions vary in length. {\it Actions: door: close, elevator: up 1 floor, shade: open, thermostat: heat  68.x to 69.y \textdegree F.} 
    }
    \medskip
...
\begin{figure}[t]
    \centering
        ...
        \centering
        \includegraphics[width=0.5\textwidth,alt={Diagram of a single energy-saving vacuum routine expressed atop RASC, where action dependencies on Acknowledgment, Start, and Completion let one routine capture the behavior cleanly.}]{RASC/figures/applications/energy-saving-vacuum-rasc.pdf}
    ...
    \begin{subfigure}[b]{\linewidth}
        \centering
        \vspace{0.2cm}
        \includegraphics[width=0.9\textwidth,alt={Diagram of the same energy-saving vacuum behavior built with traditional RPCs, requiring multiple extra routines (shown in pink) with complex or unavailable triggers; suboptimal triggers cause cross-routine conflicts or actions firing unintentionally.}]{RASC/figures/applications/energy-saving-vacuum-rpc.pdf}
        \caption{\small 
...
This chapter proposes {\it \abs} (Request-Ack-Start-Complete), a new abstraction for IoT devices, as an alternative to RPC. RASC does not replace RPC, but instead can be built atop RPC.
