...
\abs{} shares semantic roots with ROS (Robot Operating System) actionlib~\cite{ros_actionlib}, gRPC streams~\cite{grpc_streaming}, and MQTT~\cite{mqtt_v5}, which provide goal-feedback-result lifecycles or push-based notifications. However, these frameworks typically require device-side protocol changes. \abs{} is minimal.
...
Complementary HCI work surfaces this uncertainty to people rather than to the system: predictive displays for teleoperation under delay externalize \emph{when} a command takes effect and how inputs map to motion~\cite{cardinaels2026motion}, mirroring at the human interface the action-progress visibility \sysname{} provides at the systems level.
