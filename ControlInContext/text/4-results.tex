...
\paragraph{Sample and device ownership.} Our survey respondents were experienced smart home users: they owned a median of 12 smart devices (\Cref{cic:table:participants}). The most common reasons they gave for their most recent purchase were upgrading a device already in the home (38\%), added convenience (19\%), added control over some home process (19\%), and added security (17\%), and 90\% used that device at least three times a week. Participants were largely satisfied with these purchases: 67\% strongly agreed that they accomplished the goals they had in mind when buying the device, most often attributing this to added control (42\%), convenience (37\%), and security (37\%). This grounds the navigation behaviors below in the experience of users who actively live with sizeable, multi-vendor smart homes.
...
\begin{figure}[ht]
    \centering
    \includegraphics[width=0.667\textwidth,alt={Legend mapping the abbreviations used in the adjacent charts to their control schemes and control methods.}]{ControlInContext/figures/control_schemes_legend_split.pdf}
    \vfill
    \includegraphics[width=0.3\textwidth,alt={Bar chart. x-axis: control scheme, C (central) and D (other); y-axis: percentage of participants (0 to 50). Central (C) is used by about 28 percent and other (D) by about 23 percent, so the two are close with central slightly higher.}]{ControlInContext/figures/control_schemes_popularity.pdf}
    \includegraphics[width=0.36\textwidth,alt={Bar chart. x-axis: control method, R (routines), V (voice command), and S (content streaming/casting); y-axis: percentage of participants. Voice command is tallest at about 51 percent, routines about 33 percent, and content streaming the shortest at about 18 percent.}]{ControlInContext/figures/control_methods_popularity.pdf}
    \caption{\bf \small Percentage of participants using each control scheme/method.}
    \medskip
...
\begin{figure}[ht] %{0.5\textwidth}
    \centering
    \includegraphics[width=0.667\linewidth,alt={Legend mapping the abbreviations used in the adjacent charts to their control schemes and control methods.}]{ControlInContext/figures/control_schemes_legend_split.pdf}
    \par
    \includegraphics[width=0.268\linewidth,alt={Heatmap. Columns: control scheme C (central) and D (other); rows top to bottom: number of owned devices 16-38, 11-15, 4-10; cell color encodes percentage of participants (colorbar about 18 to 33). The darkest cell is central control for 16-38 devices (about 33 percent), and central stays less dark for 11-15 devices, so central use rises with more devices owned. Other control is lightest for the 11-15 device bin.}]{ControlInContext/figures/control_schemes_vs_dev_no_bins.pdf}
    \includegraphics[width=0.24\linewidth,alt={Heatmap. Columns: control method R (routines), V (voice), S (streaming); rows top to bottom: number of owned devices 16-38, 11-15, 4-10; cell color encodes percentage (colorbar about 8 to 53). The voice (V) column is darkest across all device bins (about 45 to 53 percent), routines (R) are moderate (about 30 to 38 percent) but showing an increasing trend as the number of owned devices increases, and streaming (S) is lightest, especially for the 11-15 device bin.}]{ControlInContext/figures/control_methods_vs_dev_no_bins.pdf}
    \caption{\bf \small Percentage of survey participants using central or other control schemes and several control methods vs. number of owned devices. {\it Each cell's color shows the percentage of participants falling into it who choose the respective control scheme or method. The percentages for the different options of control schemes and methods do not add up to 100\% since the participants could provide multiple answers.}}
...
\begin{figure}[ht]
    ...
    \includegraphics[width=0.667\linewidth,alt={Legend mapping the abbreviations used in the adjacent charts to their control schemes and control methods.}]{ControlInContext/figures/control_schemes_legend_split.pdf}
    \par
    \includegraphics[width=0.333\linewidth,alt={Heatmap. Columns: control scheme C (central) and D (other). Rows (top to bottom): camera, dishwasher, display, hub, light, lock, motion sensor, plug/switch, router, security, speaker, thermostat, TV, vacuum. Cell color encodes percentage; empty cells mean zero. Darkest cells: hub under C (approximately 80 percent) and router under both C and D (approximately 70 percent). Lock has a very dark D cell (non-central). Camera and display show moderate values for both. Security, vacuum, and dishwasher appear only under D (non-central). Speaker and TV appear only under C (central). Thermostat and motion sensor have no colored cells.}]{ControlInContext/figures/control_schemes_vs_dev_kind_pc.pdf}
    \includegraphics[width=0.313\linewidth,alt={Heatmap. Columns: control method R (routines), V (voice), S (streaming/casting). Rows top to bottom use the same device order as the adjacent schemes heatmap: camera, dishwasher, display, hub, light, lock, motion sensor, plug/switch, router, security, speaker, thermostat, TV, vacuum. Colorbar 0 to 100 percent. Camera: all three methods near 0. Dishwasher: R very dark (near 100 percent), V and S absent. Display: R light, V very dark (about 80 percent), S medium-dark (about 40 percent). Hub and lock: absent or near 0 for all methods. Light: R medium-dark, V medium-light. Motion sensor: R and V both very dark (about 80 percent). Plug/switch: R moderate, V moderate. Router: R dark, V and S absent. Security: R dark, S dark (about 40 percent), V absent. Speaker: R light, V moderate. Thermostat: R very dark (about 80 percent), V and S absent. TV: S very dark (about 50 percent), R and V very light. Vacuum: R medium-light, V and S absent.}]{ControlInContext/figures/control_methods_vs_dev_kind_pc.pdf}
    \caption{\bf \small Percentage of participants using each control scheme/method vs.\ device kind.
    {\it Participants may refer to a device with a different description from how they most commonly use it, leading to an occasional discrepancy between the device kind and the control method used; for example, a participant may refer to Alexa as only a smart speaker when it is also a hub~\cite{wang2020want,singh2018users, georgiev2018smart}.}}
    \medskip
...
This app-minimizing behavior is also visible at the level of the whole survey sample. \Cref{cic:fig:apps-vs-devices} relates the number of smart home apps a participant has installed to the number of devices they own. While users tend to accumulate more device-specific apps as their ecosystems grow, most kept only 1--4 apps---roughly 73\% of participants had at most four---suggesting an effort to minimize cognitive load and to centralize control when working with multiple vendors rather than installing yet another device-specific app.

\begin{figure}[ht]
    \centering
    \includegraphics[width=0.62\textwidth,alt={Scatter plot with an overlaid cumulative-distribution line. The x-axis groups survey participants by the number of smart home apps installed on their phone (0, 1-4, 5-8, more than 8); the left y-axis is the number of devices owned. Each point is a participant, colored by app-count group, and a black X marks the median number of devices in each group. The medians rise from about 5 devices at 0 apps to about 13 at 1-4 apps, 16 at 5-8 apps, and 17 at more than 8 apps. A red cumulative-distribution line (right y-axis, 0 to 1) reaches about 0.73 at the 1-4 apps group, indicating roughly three quarters of participants have at most four smart home apps installed.}]{ControlInContext/figures/smart_home_apps_cdf_vs_dev_no.pdf}
    \caption{\bf \small Survey: number of smart home apps installed vs.\ number of devices owned. {\it Points are participants (black X = per-group median); the red line is the CDF over the number of installed apps. Most participants keep only 1--4 apps even as device counts grow.}}
    \medskip
    \label{cic:fig:apps-vs-devices}
\end{figure}
...
While participants demonstrated flexibility in their control strategies, their decisions were shaped by tradeoffs between convenience, precision, cognitive effort, and integration. These tradeoffs were not static; instead, they shifted with the scale of the smart home, the types of devices involved, and the user's comfort with technology. Participants weighed these factors when choosing whether to lean toward centralized control (where a single proxy controls many devices) or physical individual device control at any point in their day. \Cref{cic:tab:control-reasons} summarizes the reasons survey participants gave for controlling a device the way they did, which we unpack in the rest of this section.

\begin{table}[t]
    \centering
    \caption{\bf \small Survey: common reasons participants gave for why they controlled a device in a particular way. {\it $N=43$ valid survey responses.}}
    \bigskip
    \label{cic:tab:control-reasons}
    \tagpdfsetup{table/header-rows={1},table/header-columns={1}}
    \small
    \renewcommand{\arraystretch}{1.2}
    \begin{tabular}{m{4cm} m{3cm} m{8cm}}
        \hline
        \textbf{Reason} & \textbf{Frequency (\%)} & \textbf{Example quote} \\
        \hline\hline
        Ease of use & 8 (19\%) & \textit{``It's the simplest and easiest way to use.''} \\ \hline
        Location of device & 9 (21\%) & \textit{``If I am nearby I use touch controls, otherwise voice commands.''} \\ \hline
        Maximize device control & 13 (30\%) & \textit{``Complete control over the entire system, without using 10 apps.''} \\ \hline
        As a backup plan & 12 (28\%) & \textit{``If Alexa isn't working to ask to turn on we can use the app.''} \\ \hline
        Force of habit & 4 (9\%) & \textit{``Voice commands are easier than to hunt out the app on my phone.''} \\ \hline
        \hline
    \end{tabular}
    \medskip
\end{table}
...
\begin{figure}[th!]
    \begin{subfigure}{\linewidth}
        \centering
        \includegraphics[width=0.7\linewidth,alt={Bar chart. x-axis: six troubleshooting steps in descending order of frequency; y-axis: percentage of participants (0 to 50). ``Investigate device power'' and ``investigate network connection'' are tied tallest at approximately 50 percent. ``Research'' is next at approximately 38 percent, ``investigate device-to-service connection'' at approximately 25 percent, ``contact support'' at approximately 19 percent, and ``customized solutions'' is shortest at approximately 7 percent. The chart illustrates a decreasing trend in user action frequency as steps become more specific or require external assistance.}]{ControlInContext/figures/survey_troubleshooting_steps_popularity.pdf}
    ...
    \begin{subfigure}{\linewidth}
        \centering
        \includegraphics[width=0.6\linewidth,alt={Directed node-link flow diagram of troubleshooting step sequences. Six circular nodes: ``Research'' (top-left), ``Investigate network connection'' (center-top), ``Investigate device power'' (top-right), ``Investigate device-to-service connection'' (center-bottom), ``Contact support'' (bottom-left), ``Customized solutions'' (bottom-right). A red inner ring marks a node as a common initial step; a green outer ring marks a common final step. ``Investigate device power'' has both rings and a self-loop (participants repeat this step), making it both a frequent start and sometimes end. ``Investigate network connection'' and ``Investigate device-to-service connection'' also have both rings. ``Research'' and ``Contact support'' have green outer rings only (frequent endpoints). Arrow thickness encodes transition frequency (scale 1 to 6 occurrences). The thickest arrows run between ``Investigate device power'' and ``Investigate network connection'' (bidirectional, about 5 to 6 occurrences) and between those two and ``Investigate device-to-service connection''; thinner arrows connect ``Research'' to ``Contact support'' and to ``Investigate network connection''.}]{ControlInContext/figures/survey_troubleshooting_step_sequence.pdf}
    ...
    \end{subfigure}
    \caption{\bf \small Troubleshooting steps results from survey responses. {\it As many are experienced users of their smart home setups, this may subtly shape the emphasis on certain troubleshooting approaches depicted here.}
    }
...
\begin{table}[t]
    \centering
    \caption{\bf \small Taxonomy of user confusion in executing smart home functionality.}
    \bigskip
    \label{cic:tab:confusion}
    \tagpdfsetup{table/header-rows={1},table/header-columns={1}}
