...
This thesis is the first to drive network and systems design for smart spaces from the needs of the people who inhabit them. It advances this vision across three \emph{independent} planes of the smart-space stack: the \emph{action plane} (RASC), a control abstraction that brings action-level observability and programmability to IoT automations; the \emph{control plane} (CoMesh), a decentralized infrastructure that removes hub bottlenecks and cloud dependence; and the \emph{management plane} (Control in Context), an empirical account of how real occupants navigate the control spectrum.
...
\begin{itemize}
    \item \textbf{The action plane: observable, programmable control (\ref{chapter:rasc}).}
    ...
    \item \textbf{The control plane: scalable, fault-tolerant control (\ref{chapter:comesh}).}
    ...
    \item \textbf{The management plane: usable control interfaces (\ref{chapter:cic}).}
...
These are independent contributions unified by a single vision---user-aware design---rather than a pipeline: no insight ``flows'' from one to the next, and none subsumes another. Together they make three planes of the smart-space stack---the \emph{action plane} (how actions are observed and programmed), the \emph{control plane} (where control logic runs), and the \emph{management plane} (how occupants view and use control interfaces)---resilient, observable, and built around the people who inhabit the space. \Cref{intro:fig:stack} situates each contribution accordingly.
