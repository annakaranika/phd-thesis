...
\input{figures/intro/architecture.tex}
...
\begin{table}[th]
    ...
    \begin{tabularx}{\linewidth}{>{\raggedright\arraybackslash}m{4.2cm}|X|>{\raggedright\arraybackslash}m{4cm}}
        ...
        \hline\hline
        CoMesh~\cite{comesh}
        & Decentralized control plane for commercial edge IoT meshes
        & System measurements \\
        \hline
...
\begin{itemize}
    ...
    \item \textbf{Observable, programmable control---middleware layer (\ref{chapter:rasc}).}
    \emph{How do control and occupants observe and program actions?}
    The RPC primitive underlying all IoT platforms is fire-and-forget, leaving developers and occupants without feedback on whether actions have actually taken effect. RASC~\cite{rasc} is a new control middleware that exposes the full action lifecycle, so that automations---and the people who write them---can observe whether an action started, completed, or failed, and program causal dependencies among actions, all without requiring any device modifications.
\end{itemize}

These contributions are independent and complementary: none subsumes another. Together they address the IoT control stack at three distinct layers---the infrastructure layer (where control logic runs), the management-interface layer (how occupants view and use control interfaces), and the control-primitive layer (how actions are observed and programmed). \Cref{intro:fig:stack} situates each contribution accordingly.
