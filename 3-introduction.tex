...
This thesis is the first to drive network and systems design for smart spaces from the needs of the people who inhabit them. It advances this vision across three \emph{independent} planes of the smart-space stack: the \emph{device actuation plane} (RASC), a control abstraction that brings action-level observability and programmability to IoT automations; the \emph{peer coordination plane} (CoMesh), a decentralized infrastructure that removes hub bottlenecks and cloud dependence; and the \emph{human interaction plane} (Control in Context), an empirical account of how real occupants navigate the control spectrum.
...
\begin{table}[th!]
    ...
    \begin{tabularx}{\linewidth}{>{\raggedright\arraybackslash}m{2.8cm}|>{\raggedright\arraybackslash}m{2.5cm}|X|>{\raggedright\arraybackslash}m{3cm}}
        \hline
        \textbf{Work} & \textbf{Plane} & \textbf{Objective} & \textbf{Evaluation} \\
        \hline\hline
        RASC~\cite{rasc}
        & Device actuation
        & Observability \& programmability abstraction for IoT device actions
        & System measurements \\
        \hline
        CoMesh~\cite{comesh}
        & Peer coordination
...
\begin{itemize}
    \item \textbf{The device actuation plane: observable, programmable control (\ref{chapter:rasc}).}
    ...
    \item \textbf{The peer coordination plane: scalable, fault-tolerant control (\ref{chapter:comesh}).}
...
These are independent contributions unified by a single vision---user-aware design---rather than a pipeline: no insight ``flows'' from one to the next, and none subsumes another. Together they make three planes of the smart-space stack---the \emph{device actuation plane} (how actions are observed and programmed), the \emph{peer coordination plane} (where control logic runs), and the \emph{human interaction plane} (how occupants view and use control interfaces)---resilient, observable, and built around the people who inhabit the space. \Cref{intro:fig:stack} situates each contribution accordingly.
