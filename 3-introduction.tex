...
\begin{figure}[th]
  \centering
  \begin{tikzpicture}[
    alt={A vertical stack of four nested layers representing the smart space control stack. From top to bottom: Control in Context (the management layer, addressing how occupants control their space); CoMesh (the coordination layer, addressing where STA logic should execute); RASC (the middleware layer, addressing what the control primitive communicates); and at the base, the smart space itself, comprising IoT devices and the mesh network. Downward arrows connect each layer to the one beneath it.},
    ...
    \node[layer, fill=orange!7, draw=orange!35] (cic) {%
      ...
      \textit{\emph{How} do occupants control their space?}\\[2pt]
      $N{=}60$ survey; $N{=}8$ interviews; \\
      physical $\longleftrightarrow$ proxy-based control spectrum};
    \node[badge] at (cic.north east) {Management};
...
\begin{table}[th]
    ...
    \begin{tabularx}{\linewidth}{>{\raggedright\arraybackslash}m{4.2cm}|X|>{\raggedright\arraybackslash}m{4cm}}
        \hline
        ...
        & User study of smart home control interface navigation
        & HCI analysis \\
        \hline
        RASC~\cite{rasc}
...
\begin{itemize}
    \item \textbf{Decentralized STA coordination---infrastructure layer (\ref{chapter:comesh}).}
    ...
    \item \textbf{Understanding how users choose their management interface (\ref{chapter:cic}).}
    \emph{Which interface do occupants use to command their devices, and why?}
    This is a \emph{management-plane} question, distinct from CoMesh's infrastructure-layer question.
...
These contributions are independent and complementary: none subsumes another. Together they address the IoT control stack at three distinct layers---the infrastructure layer (where STA logic runs), the management interface layer (which interface users choose to command their devices), and the control primitive layer (what feedback the underlying call provides). \Cref{intro:fig:stack} situates each contribution accordingly.
